...
# Config
particles = ["Pi"]
pidcuts = ["IsMuon==1.0&probe_NShared==0.0"]
# pidcuts = ["IsMuon==1.0&probe_Brunel_ANNTraining_MuonNShared==1&InMuonAcc==1"]
mags = ["up", "down"]
vars = ["Brunel_P"]
    ...
    for particle in particles:
        for pidcut in pidcuts:
            for var in vars:
                for dir in dirs:
                    with open(
                            f"../efficiency_gen/{dir}/run2-rjpsi-2016-{mag}-{particle}-{var}/effhists-Turbo16-{mag}-{particle}-{pidcut}-{var}.pkl", "rb"
                    ) as f:
